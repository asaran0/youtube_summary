"""
core/render/subtitles.py — Generate subtitle files and burn them into video.

This module is mode-agnostic: it knows nothing about "question",
"answer", "countdown" or any other mode-specific vocabulary. Instead,
each mode supplies a style_resolver callable — a function that takes a
style tag (string) and cfg, and returns {"size": int, "color": (r,g,b)}.
story_mode's resolver always returns the default style; qa_mode's
resolver knows about question/answer/countdown/try_yourself.

This is what lets core/render/ be reused unchanged when a future mode
is added — that mode just supplies its own style_resolver.

Two output files are created:
    output/<title>.srt   – standard SRT for any media player / YouTube upload
    temp/subtitles.ass   – styled ASS file used internally for burning
...
def default_style_resolver(style: str, cfg) -> dict:
    """
    Trivial style resolver used by modes that don't need per-line
    styling (e.g. story_mode) — every line gets cfg.SUBTITLE_FONT_SIZE
    and white text, regardless of the style tag.
    """
...
def generate_subtitle_files(
    ...
    _write_srt(subtitle_items, output_srt_path, cfg, style_resolver)
    log.info("SRT saved → %s", output_srt_path)

    ass_path = os.path.join(cfg.TEMP_DIR, "subtitles.ass")
    _write_ass(subtitle_items, ass_path, font_path, video_width, video_height, cfg, style_resolver)
...
def burn_subtitles(
    ...
    # ── Find font ─────────────────────────────────────────────
    # Build fonts for every size used across styles seen in this video,
    # so each line can render at its own size without per-frame loading.
    sizes_needed = {cfg.SUBTITLE_FONT_SIZE}
    distinct_styles = {s for (_, _, _, s) in subtitles}
    for style in distinct_styles:
        sizes_needed.add(style_resolver(style, cfg)["size"])

    font_cache = {}
    fallback_cache = {}
    for size in sizes_needed:
        f_pil = _load_subtitle_font(font_path, font_dir, size, ImageFont)
        f_fallback = _load_fallback_font(size, cfg, ImageFont)
        if f_pil is None:
    ...
    font_pil = font_cache[cfg.SUBTITLE_FONT_SIZE]
    fallback_font = fallback_cache[cfg.SUBTITLE_FONT_SIZE]

    bg_alpha = cfg.SUBTITLE_BG_ALPHA
    ...
    def make_frame(t):
        ...
        for (start, end, text, style) in subtitles:
            ...
            attrs = style_resolver(style, cfg)
            ...
            for raw_line in raw_lines:
                lines.extend(
                    _wrap_line_to_pixels(
                        draw, raw_line, line_font, line_fallback,
                        int(W * cfg.SUBTITLE_MAX_WIDTH_RATIO),
                    )
                )
            ...
            x0 = (W - block_w) // 2
            y0 = _subtitle_y(H, block_h, cfg)

            if bg_alpha > 0:
                overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))
                odraw = ImageDraw.Draw(overlay)
                odraw.rounded_rectangle(
                    [x0, y0, x0 + block_w, y0 + block_h], radius=6, fill=(0, 0, 0, bg_alpha),
                )
            ...
            for i, ln in enumerate(lines):
                ...
                for dx, dy in [(-2, 0), (2, 0), (0, -2), (0, 2), (-1, -1), (1, -1), (-1, 1), (1, 1)]:
                    _draw_highlighted_text(
                        ...
                        active_word, word_offset, cfg, outline=True, base_color=base_color,
                    )
                _draw_highlighted_text(
                    draw, (lx, cy), ln, line_font, line_fallback,
                    active_word, word_offset, cfg, outline=False, base_color=base_color,
                )
    ...
    final = clip.transform(lambda gf, t: make_frame(t))
...
def _write_srt(items: list[dict], path: str, cfg, style_resolver) -> None:
    lines = []
    for i, item in enumerate(items, start=1):
        attrs = style_resolver(item.get("style", "default"), cfg)
        item_max_chars = max(12, round((42 * 42) / attrs["size"]))
        text = _wrap_text(item["text"], item_max_chars)
        lines.append(str(i))
...
def _write_ass(
    ...
    ass_back = _rgba_to_ass_back(cfg.SUBTITLE_BG_ALPHA)
    margin_v = cfg.SUBTITLE_MARGIN_Y
...
Style: Default,{font_name},{cfg.SUBTITLE_FONT_SIZE},{ass_primary},&H000000FF,{ass_outline},{ass_back},1,0,0,0,100,100,0,0,3,2,1,2,20,20,{margin_v},1
...
def _flatten_to_items(chunks: list[dict]) -> list[dict]:
    ...
    "style" is whatever tag the mode assigned (or "default").
    """
    items = []
    for chunk in chunks:
...
def _load_subtitle_font(font_path: str | None, font_dir: str, size: int, image_font):
    ...
    for candidate in candidates:
        try:
            return image_font.truetype(candidate, size=size, layout_engine=image_font.Layout.RAQM)
        except Exception:
            continue
    return None


def _load_fallback_font(size: int, cfg, image_font):
    for path in cfg.FALLBACK_FONT_SEARCH_PATHS:
        if not os.path.exists(path):
            continue
        try:
            return image_font.truetype(path, size=size, layout_engine=image_font.Layout.RAQM)
        except Exception:
            continue
    return None


def _subtitle_y(video_height: int, block_height: int, cfg) -> int:
    if cfg.SUBTITLE_POSITION == "middle":
        return max((video_height - block_height) // 2, 0)
    return max(video_height - cfg.SUBTITLE_MARGIN_Y - block_height, 0)
...
def _draw_highlighted_text(
    ...
    for token in _text_tokens(text):
        ...
        if outline:
            ...
            fill = (*cfg.SUBTITLE_HIGHLIGHT_COLOR, 255)
        else:
            fill = (*base_color, 255)
